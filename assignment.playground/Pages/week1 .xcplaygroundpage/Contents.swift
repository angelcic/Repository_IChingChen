...
// git remote -v： 查看當前遠端版本庫的url。 -v   表示要求輸出更詳細的版本庫訊息。可確定設定的遠端版本庫 ＵＲＬ 是否正確。
// git branch： 查看當前所在的分支。可確定現在操作的是哪一個分支。
// fork： GitHub 中相當於 clone 的功能，能複製 GitHub 上其他人的版本庫到自己的帳號下。在 GitHub 上看到有興趣的專案可用 fork 功能將他人專案的遠端版本庫複製到自己的遠端版本庫進行修改。在不影響他人的版本庫的同時也能保有對版本庫的原作者的連結，讓看到我的版本庫的人可以知道原作者是誰。
...
//3) 在本地用 $ git remote add [Repo_Name] [Repo_URL] 指令紀錄遠端版本庫的 URL 。參數 Repo_Name 是給遠端版本庫的代稱，一般預設用 origin ，參數 Repo_URL 是檔案在 GitHub 上版本庫的 URL ，"https://github.com/(自己的GitHub帳號)/(GitHub上的版本庫名稱).git"
//4) 接著在本地用 $ git push [Repo_Name] [Branch_Name] 指令將本地的 Repository 上傳到指定的 github  版本庫及分支。他會將可透過 Branch_Name 分支上可訪問的所有 commit 複製到 GitHub 的版本庫中。
//
