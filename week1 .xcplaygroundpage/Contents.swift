...
// git log： 以新到舊列出版本庫的提交歷史紀錄。列出最近的 commit 及其相關訊息（包含提交 ID ）。發現現行版本發生過去沒有的問題，可藉由此指令查找 log ，看歷次做了什麼修改。
// git push [Repo_name] [Branch_name]： 將本地端的檔案上傳到遠端的版本庫。想把本地端的分支 commit 上傳到遠端版本庫時可用此方法上傳。
// git remote -v ：查看當前遠端版本庫的 url 。 -v 表示要求輸出更詳細的版本庫訊息。可確定設定的遠端版本庫 URL 是否正確。
// git branch： 查看當前所在的分支。可確定現在操作的是哪一個分支。
// fork： GitHub 中相當於 clone 的功能，能複製 GitHub 上其他人的版本庫到自己的帳號下。在 GitHub 上看到有興趣的專案可用此功能將他人專案的遠端版本庫複製到自己的遠端版本庫進行修改，在不影響他人的版本庫的同時也能保有對版本庫的原作者的連結，讓看到我的版本庫的人可以知道原作者是誰。
...
// 在本地創建版本庫
// 1) 在 terminal 用 $cd 指令進到想做版本控制的目錄下
// 2) 用 $ git init 在該目錄下初始化或創建新的 git 版本庫
// 3) 用 $ git add [file_name] 將檔案加入暫存區
// 4) 用 $ git commit 將暫存區的檔案存入版本庫中

// 在 GitHub 創建版本庫
// 1) 在 GitHub 右上角點選「 + 」號，並選擇「 New repository 」
// 2) 填寫版本庫的名稱，然後按下「 Create repository 」

// 連結本地和遠端版本庫，並將本地版本庫上傳至遠端版本庫
// 1) 在本地用 $ git remote add [Repo_Name] [Repo_URL] 指令紀錄遠端版本庫的 URL 。參數 Repo_Name 是給遠端版本庫的代稱，一般預設用 origin ，參數 Repo_URL 是檔案在 GitHub 上版本庫的 URL ， "https://github.com/(自己的GitHub帳號)/(GitHub上的版本庫名稱).git"
// 2) 接著在本地用 $ git push [Repo_Name] [Branch_Name] 指令將本地的 Repository 上傳到指定的 github  版本庫及分支。他會將可透過 Branch_Name 分支上可訪問的所有 commit 複製到 GitHub 的版本庫中
// 3) 若本地版本庫有更新，應先用 $ git checkout [Branch] 切換分支並更新到新的分支再上傳至遠端版本庫，確定沒有衝突後再合併到主分支
...
// When calculating circle’s area, we use the formula: 2 * radius * Pi to get area. Now, please define a variable Pi and assign a value to it. You can refer to the syntax above while do think about using  var or let and which data type it should be.
 ...
 let Pi = Double.pi // 因為 PI 在數學中有代表的固定值，不需要改變其值，因此設為常數型態
...
// 用 Double() 將整數換成浮點數後再指定給 average
...
// 命名中不能有空白鍵
// 用 Camel case 方法命名法來區隔命名中的單字，以小寫開頭，後續單字以大寫開頭
// 命名中不能以數字或特殊符號為開頭

//10. What is Type Inference in swift?

// 變數常數在初始化時不用宣告他是什麼型別的變數常數， swift 會自動從存入的資料判斷它是什麼型別的變數常數
// var iAmString: String = "String"
// var iAmString = "String" // 不用宣告為 String ， swift 也知道他是 String 型別的變數

//11. What is the problem about this piece of code?
//var phoneNumber = 0987654321，
//phoneNumber = "Hello world."

//在初始化變數 phoneNumber 時指定存入 Int 型別的資料，因此被默認為 Int 型別的變數
//因此之後無法再存入不同型別的資料，例如 String
...
// 因為myFriend這個陣列中目前只有4筆資料，而陣列的 index 是從 0 開始計算的，因此最多只能取出第四筆資料，也就是 index 3，超過這個範圍 Swift 編譯器會跳出 Index out of range 的錯誤提示，因此無法查詢在 myFriend 陣列中查詢 index 5
//myFriend.count = 4
//myFriend[5] // Error
...
// 可以直接將 key 對應的 value 改為 nil
myCountryNumber["US"] = nil
// 或是呼叫 removeValue(forKey: (key))方法
